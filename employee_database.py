...
def space():
    """creates spacing on comman line"""
    print('_' * 75)
...
class Entry():
    ...
    def name_first(self):
        """Allows user to assign first name to Entry instance"""
        self.first = input("Enter your first name:\n>").title()
        clear()
        return self.first

    def name_last(self):
        """Allows user to assign last name to Entry instance"""
        self.last = input("Enter your last name:\n>").title()
        clear()
        return self.last

    def name_task(self):
        """Allows user to assign task to Entry instance"""
        self.task = input("Enter the task name:\n>")
        clear()
        return self.task

    def name_time(self):
        """Allows user to assign time to Entry instance"""
        value = True
        while value:
            self.time = input("""How much time was spent on your task
in minutes:\n> """)
            try:
                int(self.time)
                str(self.time)
...
(ex. 30)\n""")
    ...
    def name_notes(self):
        """Allows user to assign notes to Entry instance"""
        self.notes = input("""Enter any desired associated notes,
or enter none if you have no additiionl notes\n>""")
        clear()
        return self.notes

    def name_date(self):
        """Allows user to assign date to Entry instance"""
        choice = True
        while choice:
...
(enter date as year-mm-dd-> ex.2018-04-28)\n\n>""")
    ...
    def entry_displayed(self):
        """displays Entry with associated attributes
           and lets user decide if they want to save
           entry to database
           """
        print(f"""Entry:\n
...
Date: {self.date} \n""")
        space()
        if input("""Confirm that would like to save your entry by entering yes,
if you do not want to save enter no.\n """).lower() != 'no':
            Employee.create(
                first_name=self.first,
                last_name=self.last,
                task_name=self.task,
                time_spent=self.time,
...
def process_entry():
    """runs functions to create instance"""
    entry = Entry()
    entry.name_first()
    entry.name_last()
    entry.name_task()
...
class Search():
    """creates class object with associated methods that allow
       user at command line to search entries from database by
       name, key term associated with task/notes, time spent on
     task or dates
       """
    ...
    def search_name(self):
            """gives list of employee names with entries in database
               and returns entries associated with first or last name
               entered by user at command line"""
            duplicates = []
            full_name = Employee.select(
                Employee.first_name, Employee.last_name).get()
            print("""Listed below are employee names
with associated database entries:\n""")
            for name in full_name.select():
                print(name.first_name + ' ' + name.last_name)
            print('\n')
            space()
            found_entries = Employee.select()
            self.prior_entries = input("""Please enter the first or last name of the employee entry
you would like to view.\n> """).title()
    ...
    def search_date(self):
        """Allows user to search database entries by date"""
        lis = []
...
(enter date as year-mm-dd-> ex.2018-04-28)\n\n>""")
        ...
        clear()
        self.entries = found_entries.where(
            Employee.log_date == self.prior_entries
            )
        return self.entries

    def search_time(self):
        """Allows user to search database entries by time"""
        duplicates = []
        times = Employee.select(Employee.time_spent).get()
        print("""Listed below are employee time logs
with associated database entries:\n""")
        ...
        space()
        found_entries = Employee.select()
        self.prior_entries = input("""Enter the time (minutes) of the entry
you would like to search, only us numerical data.
(ex. for 12 minutes enter --> 12)\n
> """)
        clear()
        self.entries = found_entries.where(
            Employee.time_spent == self.prior_entries
            )
        if len(self.entries) >= 1:
            return self.entries

    def search_pattern(self):
        """Allows user to search database entries by pattern
           associated with entry task/notes"""
        found_entries = Employee.select()
...
to search the employee database in regards to task name or notes:\n> """)
        clear()
        self.entries = found_entries.where(
            (Employee.task_name.contains(self.prior_entries)) | (Employee.task_notes.contains(self.prior_entries)))
        if len(self.entries) >= 1:
            return self.entries

    def search_display(self):
        """Displays database search findings by entry"""
        if len(self.entries) >= 1:
    ...
    '''.format(entry.first_name, entry.last_name, timestamp, entry.task_name, entry.time_spent, entry.task_notes))
            ...
            print(f"""\nINVALID ENTRY: {self.prior_entries} was not found in the database.
    \n""")


def process_search():
    """searches database and displays entries"""
...
def menu_loop():
    """menu loop that continues to run program until
